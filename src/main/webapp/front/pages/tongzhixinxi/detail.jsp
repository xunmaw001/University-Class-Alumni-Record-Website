		...
		<script>
			var vue = new Vue({
				...
				methods: {
					...
					onAcrossTap(acrossTable,statusColumnName,tips,statusColumnValue){
						localStorage.setItem('crossTable',`tongzhixinxi`);
					...
					storeUp() {
						layui.http.request('storeup/list', 'get', {
							...
							layui.http.requestJson('storeup/save', 'post', {
								...
								name: this.title,
							    type:1,
								picture:vue.detail.tongzhifengmian,
								refid: this.detail.id,
								tablename: this.detailTable
							}, function(res) {
								layer.msg('收藏成功', {
			...
			layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage'], function() {
				...
				http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function(res) {
					// 详情信息
					vue.detail = res.data
					vue.title = vue.detail.tongzhibiaoti;
					// 轮播图片
					vue.swiperList = vue.detail.tongzhifengmian ? [http.baseurl+vue.detail.tongzhifengmian.split(",")[0]] : [];
					// var swiperItemHtml = '';
